...
2. Editez le fichier Program.cs avec l'URL correspondant à votre ville (variable `doctolibSearchUrl`). Si vous souhaitez ignorer les créneaux de certains centres de vaccination, vous pouvez les mentionner dans la variable `centersToIgnore`. Il y a également une variable `centerDepartmentsToIgnore` pour sépcifier des départements entiers (voire des codes postaux) dont on souhaite ignorer les disponibilités des centres de vaccination.
